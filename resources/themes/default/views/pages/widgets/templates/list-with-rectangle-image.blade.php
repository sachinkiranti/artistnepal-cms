...
@endphp
<div data-component="list-with-rectangle-image" class="card-news sahitya-bibidh" style="{{ config('wizard.widget.every_div_style') }}">

    @include('pages.shared.home.editor')

    <div class="title bg-teal">
        <h4>{{ $title ?? '' }}</h4>
        @isset($category)
    ...
    @forelse ($data['posts'] as $post)
        ...
        @if($loop->iteration < ($no_of_rectangle_image ?? 1) + 1)
                ...
                @if ($post->isThumbnailVisible())
                    @if ($post->image)
                        <div class="bichar-img hover-holder">
                            <a href="{{ route('post.single', $post->unique_identifier) }}" title="{{ $post->title }}">
                                <img class="lazy" data-src="{{ get_thumbnail( $post->image, $post->unique_identifier ) }}" alt=""></a>
                        </div>
                    @endif
                @endif
