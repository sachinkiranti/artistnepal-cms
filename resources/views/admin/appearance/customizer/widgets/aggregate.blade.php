@include('admin.appearance.customizer.widgets.partials.widget-top', [
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-12 {{ $errors->has('entity')?'has-error':'' }}">
            {!! Form::select('entity', $entities, null, [ 'class' => 'form-control', ]) !!}
    ...
    <div class="form-group row">
        <label class="col-sm-12 col-form-label is_required">Rules <span class="required">*</span></label>
        <div class="col-sm-12 {{ $errors->has('rules')?'has-error':'' }}">
            {!! Form::select('rules', $aggregates, null, [ 'class' => 'form-control', ]) !!}
            @if($errors->has('rules'))
                <label class="has-error" for="rules">{{ $errors->first('rules') }}</label>
            @endif
        </div>
    </div>
